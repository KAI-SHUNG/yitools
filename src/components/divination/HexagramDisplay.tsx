...
export default function HexagramDisplay({ yaos, hexagramName, najia, liuChong, liuHe, showShiYing = true, compact = false }: Props) {
  ...
  return (
    <div className="flex flex-col items-center">
      {hexagramName && (
        <p className={`text-ink-black tracking-widest font-medium mb-1 ${compact ? 'text-base' : 'text-lg'}`}>
          {hexagramName}
        </p>
      )}
        ...
        {sorted.map((yao) => {
          ...
          return (
            <div key={yao.position} className="flex items-center" style={{ minWidth: rowMinW }}>
              ...
              {/* Right: 世/应 then ○/× */}
              <div className="flex items-center gap-1.5 text-xs" style={{ width: rightW }}>
                ...
                {yao.isChanging && (
                  <span className="text-lake-green font-bold" style={{ fontSize: compact ? 12 : 14 }}>
                    {yao.polarity === 'yang' ? '○' : '×'}
                  </span>
                )}
              </div>
